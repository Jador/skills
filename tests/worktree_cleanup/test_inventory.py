...
def test_schema1_and_schema2_produce_identical_inventory(
    tmp_path, worktree_paths, run_script, fake_bin
):
    """Same input worktree state, both schemas -> byte-identical normalized inventory."""
    entries = _entries(worktree_paths)

    fixture1 = tmp_path / "wt_list_schema1.json"
    fixture1.write_text(json.dumps(_schema1_fixture(entries)))
    fixture2 = tmp_path / "wt_list_schema2.json"
    fixture2.write_text(json.dumps(_schema2_fixture(entries)))

    result1 = run_script(
        ["--format=json"], wt_bin=fake_bin["wt"], gh_bin=fake_bin["gh"], wt_json_file=fixture1
    )
    result2 = run_script(
        ["--format=json"], wt_bin=fake_bin["wt"], gh_bin=fake_bin["gh"], wt_json_file=fixture2
    )
...
def test_text_format_reports_dirty_and_ignored(
    ...
    entries = _entries(worktree_paths)
    fixture_file = tmp_path / "wt_list.json"
    fixture_file.write_text(json.dumps(_schema1_fixture(entries)))

    result = run_script(
        [], wt_bin=fake_bin["wt"], gh_bin=fake_bin["gh"], wt_json_file=fixture_file
    )
