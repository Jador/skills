...
**`comment_thread` → spawn a comment-check sub-agent.** Use the Agent (sub-agent) tool with `model: sonnet` — comment-check work is bounded and sonnet handles it well, and the inline `model` param is honored per-spawn. Do **not** use `subagent_type: "fork"` for this spawn: fork ignores the inline `model`. Load the worker prompt from `${CLAUDE_SKILL_DIR}/assets/comment-check-prompt.md` and use it as the sub-agent's system prompt / instructions. Pass the event payload as the sub-agent's user message — the JSON line you just parsed, verbatim, inside a fenced ```json block. If freeform instructions were captured during Argument Parsing, prepend them to the user message above the JSON block under a `Freeform instructions:` header so the sub-agent sees them in context. Likewise, if handoff context was loaded (see "Load Handoff Context" above), prepend it under a `Handoff context:` header so the worker knows the author's intent and deliberate choices.

**`build_failure` → spawn a build-check sub-agent.** Same shape as above, including `model: sonnet` on the Agent tool call (and, likewise, never `subagent_type: "fork"`), but load the worker prompt from `${CLAUDE_SKILL_DIR}/assets/build-check-prompt.md` instead. Pass the event payload as the user message in a fenced ```json block, with any freeform instructions prepended exactly as for `comment_thread`.
