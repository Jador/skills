...
5. **Send the unmodified four-section report to `{{WORKER_NAME}}` with `SendMessage`.** Do not rely on plain output — plain output goes to the orchestrator, not the worker. Send exactly one report per checkpoint: if you are resumed again without a new checkpoint message, do not resend it. Then end your turn. You may be resumed again.
