...
# cmd_scan currently wires up the inventory above; it does not yet
# categorize or cache a plan.
# ---------------------------------------------------------------------------
...
cmd_scan() {
  ...
  # TODO(later task): cross-check GitHub PR status via `${GH_BIN}`,
  # categorize each worktree, and write the plan to the cache path. Until
  # then, --format=json surfaces the raw normalized inventory, and the text
  # report is a plain per-worktree listing.
  if [[ "$FORMAT" == "json" ]]; then
    printf '%s\n' "$inventory"
  else
    printf '%s\n' "$inventory" | jq -r '
